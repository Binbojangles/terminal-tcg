...
impl Shop {
    fn buy_cards(&mut self, distributor: &Distributor) {
        ...
        if choice > 0 && choice <= distributor.cards.len() {
            ...
            if self.cash >= card.base_price {
                self.inventory.cards.entry(card.name.clone()).or_insert((0, card.clone()));
    ...
    fn set_card_price(&mut self) {
        ...
        std::io::stdin().read_line(&mut card_name).expect("Failed to read line");
        let card_name = card_name.trim();

        if let Some((quantity, price)) = self.inventory.cards.get_mut(card_name) {
            loop {
                println!("Enter the new price for {} (current price: ${:?})", card_name, price.base_price);
                let mut new_price = String::new();
                std::io::stdin().read_line(&mut new_price).expect("Failed to read line");
                ...
                if new_price > 1000 {
                    ...
                    println!("The price of {} is now ${:?}. Quantity in stock: {}", card_name, price.base_price, quantity);
                    break;
                }
            }
        } else {
    ...
    fn sell_cards(&mut self) {
        ...
        println!("{} enters the shop with a budget of ${}.", customer.name, customer.budget);

        let mut affordable_cards: Vec<(&String, &Card)> = self.inventory.cards.iter()
            .filter(|(_, (_, card))| card.base_price <= customer.budget)
            .map(|(name, (_, card))| (name, card))
            .collect();
        
        if affordable_cards.is_empty() {
            println!("{} can't afford any of the cards in the shop and leaves disappointed.", customer.name);
            return;
        }

        affordable_cards.sort_by(|(_, price1), (_, price2)| price2.base_price.cmp(&price1.base_price));

        let preferred_cards: Vec<(&String, &Card)> = affordable_cards.clone().into_iter()
            .filter(|(name, _)| {
                if let Some((_, (_, card))) = self.inventory.cards.get_key_value(&*name) {
                    customer.preferences.contains(&card._rarity)
                } else {
                    false
                }
            })
        ...
        *quantity -= 1;
        ...
        println!("{} bought {} for ${:?}!", customer.name, card_to_buy.0, card_to_buy.1.base_price);
...
fn main() {
    let mut shop = Shop {
        ...
        inventory: Inventory { cards: HashMap::new() },
        cash: 1000,
        level: 1,
        reputation: 0,
    };
    ...
    loop {
        ...
        for (card, (quantity, price)) in &shop.inventory.cards {
            println!("  - {} (Qty: {}, Price: ${:?})", card, quantity, price.base_price);
        }
